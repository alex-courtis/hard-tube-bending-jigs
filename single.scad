...
// applied to nut_width * nut_hole_multiplier
shaft_multiplier = 1.02; // [1:0.01:5]
...
module extrude_straight(text) {
  shaft_dx = wall_width + channel_width / 2;
  shaft_dy1 = straight_l / 2 - shaft_radius;
  shaft_dy2 = -straight_l / 2 + shaft_radius;
  ...
  difference() {
    ...
    union() {
      ...
      // bolt shafts
      color(c="darkblue") {
        translate(v=[shaft_dx, shaft_dy1, wall_width])
          cylinder(r=shaft_radius, h=channel_height - wall_width);
        translate(v=[wall_width, shaft_dy1 - wall_width / 2, wall_width])
          cube([channel_width, wall_width, channel_height - wall_width], center=false);
      }
      color(c="steelblue") {
        translate(v=[shaft_dx, shaft_dy2, wall_width])
          cylinder(r=shaft_radius, h=channel_height - wall_width);
        translate(v=[wall_width, shaft_dy2 - wall_width / 2, wall_width])
          cube([channel_width, wall_width, channel_height - wall_width], center=false);
      }
    }
    ...
    // bolt holes
    translate(v=[shaft_dx, shaft_dy1, 0])
      bolt_hole(top=text);
    translate(v=[shaft_dx, shaft_dy2, 0])
      bolt_hole(top=text);
  }
}
